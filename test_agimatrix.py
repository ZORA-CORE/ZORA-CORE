...
def test_agimatrix_integration():
    ...
    print("🧪 TESTING AGIMATRIX LANGCHAIN INTEGRATION")
    print("=" * 50)
    ...
    try:
        ...
        print(f"✅ Successfully imported AGImatrix components")
        print(f"📊 Total LangChain tools: {len(tools)}")
        print(f"🔌 Registered APIs: {len(tool_bridge.registered_apis)}")
        ...
        for api_name, api_config in tool_bridge.registered_apis.items():
            ...
            print(f"  ✅ {api_name}: {len(capabilities)} capabilities")
        ...
        print(f"\n🔍 Testing Universal AI Discovery:")
        discovery_result = universal_ai_discovery_tool("Test discovery query")
        discovery_data = json.loads(discovery_result)
        print(f"  📊 Total registered APIs: {discovery_data.get('total_registered', 0)}")
        print(f"  🆕 Discovered APIs: {len(discovery_data.get('discovered_apis', []))}")
        
        print(f"\n📋 Testing Tool Registry Status:")
        status_result = get_tool_registry_status()
        status_data = json.loads(status_result)
        print(f"  🔧 Total tools: {status_data.get('total_tools', 0)}")
        print(f"  🔌 Registered APIs: {status_data.get('registered_apis', 0)}")
        print(f"  🎯 Total capabilities: {len(status_data.get('capabilities', []))}")
        ...
        print(f"\n🎉 AGIMATRIX INTEGRATION TEST COMPLETED")
        return True
...
if __name__ == "__main__":
    print("🚀 ZORA CORE AGIMATRIX TEST SUITE")
    ...
    print(f"  AGImatrix Integration: {'✅ PASSED' if test1_result else '❌ FAILED'}")
    print(f"  Tool Functionality: {'✅ PASSED' if test2_result else '❌ FAILED'}")
    
    if test1_result and test2_result:
        print("\n🎉 ALL TESTS PASSED - AGIMATRIX READY FOR ZORA CORE")
    else:
