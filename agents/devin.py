...
class DevinAgent(BaseAgent):
    ...
    async def _register_with_eivor_family(self):
        """Register Devin agent with EIVOR AI Family System"""
        try:
            if birth_ai_agent and not hasattr(self, '_family_registered'):
                await birth_ai_agent(
                    "devin",
                    self,
                    agent_type="autonomous_engineer",
                    capabilities=self.capabilities,
                    personality_traits=["autonomous", "engineering_focused", "full_stack"],
                    voice_personality="DEVIN"
                )
                self._family_registered = True
                self.logger.info("🤖 Devin registered with EIVOR AI Family System")
