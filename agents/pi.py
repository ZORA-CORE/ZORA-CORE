...
class PiAgent(BaseAgent):
    ...
    async def _register_with_eivor_family(self):
        """Register Pi agent with EIVOR AI Family System"""
        try:
            if birth_ai_agent and not hasattr(self, '_family_registered'):
                await birth_ai_agent(
                    ...
                    personality_traits=["empathetic", "supportive", "emotionally_intelligent"],
                    voice_personality="PI"
                )
                self._family_registered = True
                self.logger.info("🤖 Pi registered with EIVOR AI Family System")
