...
try:
    ...
    print("⚠️ Voice integration modules not available")
...
class ZoraVoiceCore:
    ...
    def speak(self, expression):
        ...
        print(f"[VOICE - {expression['speaker']}] ({expression['emotion']}) » {expression['message']}")
        print(f"Tone: {expression['tone']}")
        print(f"Time: {expression['timestamp']}")
        
        if self.voice_synthesis_enabled and self.voice_engine:
            print("🎤 Neural TTS synthesis available")
        else:
            print("🔇 Neural TTS not available - symbolic mode only")
        print("---")
    ...
    async def synthesize_expression(self, expression):
        ...
        if not self.voice_synthesis_enabled or not self.voice_engine:
            print(f"⚠️ Voice synthesis not available for {expression['speaker']}")
            return None
        ...
        try:
            ...
            if audio_data:
                print(f"🎤 Synthesized voice for {expression['speaker']}: {expression['emotion']} tone")
                return audio_data
            else:
                print(f"⚠️ Voice synthesis failed for {expression['speaker']}")
                return None
